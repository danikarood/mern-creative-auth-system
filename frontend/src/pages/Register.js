...
export default function Register({ onToggleForm }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (emojiPattern.length === 0) {
      setError("Please pick your security pattern sequence.");
      return;
    }
    ...
    try {
      const response = await fetch(`${API_URL}/api/auth/register`, {
        ...
        body: JSON.stringify({ username, email, password, confirmPassword, emojiPattern })
      });
      ...
      setError("Server validation handshake error.");
    }
  };
  ...
  return (
    <div className="auth-card-container">
      ...
      <p className="auth-subtitle">Join our high-fidelity secure ecosystem</p>
      ...
      <form onSubmit={handleSubmit}>
        <div className="auth-form-group">
          ...
          <input type="text" className="auth-form-input" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Username" />
        </div>
        ...
        <div className="auth-form-group">
          ...
          <div className="password-input-wrapper">
            <input type={showConfirmPassword ? "text" : "password"} className="auth-form-input" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="••••••••" />
            <button type="button" className="password-toggle-visibility-btn" onClick={() => setShowConfirmPassword(!showConfirmPassword)} aria-label="Toggle confirm password visibility">
              {showConfirmPassword ? (
                <svg fill="currentColor" viewBox="0 0 16 16"><path d="M13.359 11.238C15.06 9.72 16 8 16 8s-3-5.5-8-5.5a7.028 7.028 0 0 0-2.79.588l.77.771A5.944 5.944 0 0 1 8 3.5c2.12 0 3.879 1.168 5.168 2.457A13.134 13.134 0 0 1 14.828 8c-.058.087-.122.183-.195.288-.335.48-.83 1.12-1.465 1.755-.165.165-.337.328-.517.486l.708.709z"/><path d="M11.297 9.176a3.5 3.5 0 0 0-4.474-4.474l.823.823a2.5 2.5 0 0 1 2.829 2.829l.822.822zm-2.943 1.299.822.822a3.5 3.5 0 0 1-4.474-4.474l.823.823a2.5 2.5 0 0 0 2.829 2.829z"/><path d="M3.35 5.47c-.18.16-.353.322-.518.487A13.134 13.134 0 0 0 1.172 8l.195.288c.335.48.83 1.12 1.465 1.755C4.121 11.332 5.881 12.5 8 12.5c.716 0 1.39-.133 2.02-.36l.77.772A7.029 7.029 0 0 1 8 13.5C3 13.5 0 8 0 8s.939-1.721 2.641-3.238l.708.709zm10.296 8.884-12-12 .708-.708 12 12-.708.708z"/></svg>
              ) : (
                <svg fill="currentColor" viewBox="0 0 16 16"><path d="M10.5 8a2.5 2.5 0 1 1-5 0 2.5 2.5 0 0 1 5 0z"/><path d="M0 8s3-5.5 8-5.5S16 8 16 8s-3 5.5-8 5.5S0 8 0 8zm8 3.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7z"/></svg>
        ...
        <h3 className="security-section-title">Pattern Identifier Verification</h3>
        <div className="pattern-preview-bar">
          {emojiPattern.length === 0 ? (
            <span className="preview-placeholder">Your custom pattern sequence...</span>
          ) : (
        ...
        <button type="submit" className="auth-submit-btn">Sign Up Dashboard</button>

        <div className="auth-switch-footer">
          Already have an account?
          <button type="button" onClick={onToggleForm} className="auth-switch-link-btn">Sign In</button>
        </div>
      </form>
    </div>
  );
